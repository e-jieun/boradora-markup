...
title.style.color = `${colorObj.colorYl}`;
drama.style.width = '50vw';
drama.style.textAlign = 'center';
drama.style.color = `${colorObj.colorBp}`;
...
let colorArr = ['https://picsum.photos/200/300?random=1', 'https://picsum.photos/1000/1000?random=2', 'https://picsum.photos/200/300?random=3', 'https://picsum.photos/200/300?random=4', 'https://picsum.photos/200/300?random=5'];
// *slideChild style
for (let i = 0; i < slideChild.length; i++) {
  slideChild[i].setAttribute('style', `width: 100vw; height: 100vh; border: 1px solid red; opacity: 0.8; min-width: 100vw;`);
  slideChild[i].innerHTML = `<img src=${colorArr[i]}>`;

  const slideChildImg = slideChild[i].firstElementChild;
  console.log(slideChildImg);
  slideChildImg.setAttribute('style', `width: 100vw; height: 100vh; object-fit: cover;`)
...
const playBtn = buttonCon.children[1];
console.log(leftBtn);
...
// playBtn style
console.log(playBtn);
playBtn.setAttribute('style', `width: 50px; height: 50px; background = null; border: none; transform: rotate(180deg); border-radius: 50%;`);
